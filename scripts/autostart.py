...
"When I boot my system and I say Hey Zoey" needs the daemon to be running after
...
FOUR OPTIONS. THE STARTUP FOLDER WINS.
  ...
  1. STARTUP FOLDER SHORTCUT            <- chosen
     %APPDATA%\\...\\Start Menu\\Programs\\Startup\\Zoey.lnk
...
SHORTCUT = STARTUP_DIR / "Zoey.lnk"
LOG_DIR = REPO / "data" / "logs"
LAUNCHER = REPO / "scripts" / "start-zoey.cmd"
...
REM Delete the shortcut in shell:startup to stop Zoey starting at login,
...
if not "%ERRORLEVEL%"=="0" (
  echo.
  echo Zoey exited with %ERRORLEVEL%. The log is at:
  echo   %LOGDIR%\\daemon-%STAMP%.log
  pause
)
"""
...
$lnk.Description = 'Zoey Core daemon'
$lnk.WindowStyle = 7
$lnk.Save()
"""
...
def install(voice: bool, stt_model: str) -> int:
    ...
    print("  or Task Manager > Startup > Zoey > Disable")
    return 0
...
if __name__ == "__main__":
    ap = argparse.ArgumentParser(description="Start Zoey at login")
